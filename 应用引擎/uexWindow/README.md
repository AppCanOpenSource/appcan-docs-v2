...
| extraInfo | Number | 否 | 扩展参数，设置值时，animDuration参数必传，json格式如下：       {"extraInfo":{"opaque":"true","bgColor":"#011","hardware":0}} 

各字段含义如下:

|参数|是否必须|说明|
|---|-------|----|
|opaque|可选|是否透明true/false 默认为false|
|bgColor|可选|背景色，支持图片和颜色，格式为 #fff、#ffffff、rgba(r,g,b,a)等，图片路径支持 res:// wgt:// 等AppCan协议路径 
|hardware|可选|是否开启硬件加速，0：否，1：开启（仅Android）
