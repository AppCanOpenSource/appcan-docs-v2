/*! appcan v1.1.0 |  from www.appcan.cn */
...
//    Zepto.js
//    create by jiaobingqian
//    定义$.fn.jQuery,adapt bootstrap
;(function($){
  $.fn.jQuery = '2.0.0 virtual jQuery version!';
})(Zepto);
...
/*
    ...
    created:2016.07.07
    update:2016.07.07 by jiaobingqian
*/
/* global window */
;(function (root, factory) {
    ...
    */
    var extendFunc = function(name,factory){
        var appcan = window.appcan || {};
    ...
    //代码入口
    var use = function(modules,factory){
...
appcan && appcan.define('database',function($,exports,module){
    ...
    var database = {
        ...
        */
        open:function(name){
            var db = {id:uexDataBaseMgr.open(name)};
			if(db){
				db.select=function(sql,callback){
					uexDataBaseMgr.select(this.id,sql, function (error,data) {
						callback(error,data);
					});
				}
				db.exec=function(sql,callback){
					uexDataBaseMgr.sql(this.id,sql, function (error,data) {
						callback(error,data);
					});
				}
				db.transaction=function(sqlFun,callback){
					uexDataBaseMgr.transactionEx(this.id,sqlFun, function(error) {
						callback(error);
					});
				}
				db.close=function(){
					uexDataBaseMgr.close(this.id);
				}
			}
			return db;
        },
        ...
        exec:function(db,sql,callback){
            db.exec(sql,callback);
        },
        transaction:function(db,sqlFun,callback){
...
appcan && appcan.define('request', function($, exports, module) {
    ...
    function processCompleteResult(xhr, req, status, result, requestCode, response, deferred) {
    ...
    function processProgress(progress, xhr, req) {
        var settings = req.settings;
    ...
    function ajax(options) {
		...
		//xhr.open(settings.type, settings.url, async, settings.username, settings.password)
        var req = {id:xhr.create({method:settings.type,url:settings.url,timeout:settings.timeout})};//httpId, settings.type, settings.url, settings.timeout);
        ...
        var mime = settings.accepts[dataType],
            ...
            nativeSetHeader = function(req, headers) {
                var toHeader = {};
                var fromHeader = null;
                ...
                xhr.setHeaders(req.id, JSON.stringify(toHeader));
            },
            addAppVerify = function(settings) {
                if (settings.appVerify === true) {
                    //添加app 认证头 修复模拟器不支持setAppvVerify 方法
                    xhr.setAppVerify && xhr.setAppVerify(req.id, 1);
                }
                if (settings.appVerify === false) {
                    //添加app 认证头 修复模拟器不支持setAppvVerify 方法
                    xhr.setAppVerify && xhr.setAppVerify(req.id, 0);
                }
            },
            //更新证书信息
            updateCertificate = function(settings) {
                ...
                xhr.setCertificate && xhr.setCertificate(req.id, certi.password || '', certi.path);
            },
            abortTimeout;
        //绑定相应的配置
        ...
        if (settings.data && settings.contentType === false) {
            for (name in settings.data) {
                ...
                if (appcan.isPlainObject(settings.data[name])) {
                    if (settings.data[name].path) {
                        //上传文件数据
                        xhr.setPostData(req.id, "1", name, settings.data[name].path);
                    } else {
                        xhr.setPostData(req.id, "0", name, JSON.stringify(settings.data[name]));
                    }
                } else {
                    //添加普通数据
                    xhr.setPostData(req.id, "0", name, settings.data[name]);
                }
            }
        } else {
            ...
            if (settings.data) {
                xhr.setBody(req.id, settings.data ? settings.data : null);
            }
        }
        //兼容前端中断请求返回错误提示
        ...
        if (offlinedata != null) {
            ...
            if (dataObj[offlineUrl]) {
                if (dataObj[offlineUrl]['data']) {
                    ...
                    appcan.file.remove(localFilePath);
...
appcan && appcan.define('download',function($,exports,module){
    function createDownloader(callback){
        var req = {id:uexDownloaderMgr.create()};
		req.setHeaders = function(headers){
			uexDownloaderMgr.setHeaders && uexDownloaderMgr.setHeaders(this.id,JSON.stringify(headers));
		}
		req.download=function(options){//url,savePath,breakPoint,progress,successCall,errorCall,cancelCall
			if(!options.url) throw new Error('url is undefined!');
			if(!options.savePath) throw new Error('savePath is undefined!');
			options.range = parseInt(options.range ,10);
			options.range = isNaN(options.range)?'0':options.range;
			//绑定下载状态通知函数
			uexDownloaderMgr.download(this.id,options.url,options.savePath,options.range,function(fileSize,percent,status){
				if(status == 0){//DownLoading
					options.progress && appcan.isFunction(options.progress) &&  options.progress(this,fileSize,percent,status);
				}else if(status == 1) {// finish download 
					options.success && appcan.isFunction(options.success)  && options.success(this,fileSize,percent,status);
				}else if(status == 2) {// download error
					options.error && appcan.isFunction(options.error)  && options.error(this,fileSize,percent,status);
				}else if(status == 3){// cancel download
					options.cancel && appcan.isFunction(options.cancel)  && options.cancel(this,fileSize,percent,status);
				}
			});
		}
		req.close = function(){
    ...
    iCache.prototype = {
        ...
        _download : function(session) {
			...
			if(appcan.file.exists(fpath))
			{
				...
				session.dlId = appcan.download.create(null, function(err, data, dataType, req) {
					if (!err) {
						session.header && appcan.download.setHeaders(req, session.header);
						appcan.download.run(req, session.url, fpath, '0', downloadCB,downloadCB,downloadCB,downloadCB)
					} else
						downloadCB(req, 0, 0, 4)
				}); 
			}           
        },
        run : function(option) {
...
;(function(factory){
    ...
    if(!$.fn.hasOwnProperty('lazyload')){
        ...
        $.fn.lazyload = function(options){
            ...
            isScrollTypeEvent = isScrollEvent || options.event == 'scrollstart' || options.event == 'scrollstop'|| options.event == 'touchmove'
