...
appcan.define("icache", function($, exports, module) {
    ...
    function iCache(option) {
        ...
        uexFileMgr.cbGetFileRealPath = function(opCode, dataType, path) {
            self.realpath = path;
            self.emit("NEXT_SESSION");   //触发NEXT_SESSION绑定时间
        }
    ...
    iCache.prototype = {
        ...
        _next : function() {
            ...
            if (self.running.length >= self.option.maxtask)
                return;
            var session = self.waiting.shift();   //shift() 方法用于把数组的第一个元素从其中删除，并返回第一个元素的值。
            if (!session)
                return;
        ...
        _download : function(session) {
            ...
            uexFileMgr.isFileExistByPath(session.id, fpath);  //根据文件路径判断文件是否存在
        },
        run : function(option) {
            var self = this;
            var session = $.extend({
